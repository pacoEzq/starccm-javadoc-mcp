...
public class WallYplusAudit extends StarMacro {
    ...
    public void execute() {
        ...
        // getFunction() on a miss returns a NullFieldFunction, never null, so a
        // null guard is dead code — use hasFunction() for the existence check.
        FieldFunctionManager ffm = sim.getFieldFunctionManager();
        if (!ffm.hasFunction("WallYplus")) {
        ...
        FieldFunction yplus = ffm.getFunction("WallYplus");
